...
class TextProductCommon(object):
    ...
    def hazardTimePhrases(self, hazard, creationTime, prefixSpace=True):
        '''
        ...
        algorithms.
        '''
        timeWords = self.getTimingPhrase(hazard, creationTime)
    ...
    def actionControlWord(self, hazard, issuanceTime):
        ...
        if not hazard.has_key('act'):
            self.logger.error("Error!  No field act in hazard record.")
            return "<noaction>"
    ...
    def getHeadlines(self, hazardList, productID, creationTime):
        headlineStr = ""
        ...
        while len(hList) > 0:
            ...
            if (hazard['act'] != 'EXP' and creationTime >= hazard['endTime']) or \
            (hazard['act'] == 'EXP' and creationTime > 30*60 + hazard['endTime']):
                hList.remove(hazard)
                continue # no headline for expired hazards
            ...
            # add on the action
            actionWords = self.actionControlWord(hazard, creationTime)
            ...
            if len(hazStr):
                ...
                localStr = self.hazard_hook(
                  None, None, hazard['phen'], hazard['sig'], hazard['act'],
                  hazard['startTime'], hazard['endTime'])  # May need to add leading space if non-null 
    ...
    def getTimingPhrase(self, hazRec, issueTime, stype=None, etype=None):
        ...
        timeZones = self.hazardTimeZones(hazRec['officeid'])
        ...
        if type(stype) is types.MethodType:
            for tz in timeZones:
                newType, info = stype(
                    issueTime, hazRec['startTime'], tz, 'startTime')
        ...
        if type(etype) is types.MethodType:
            for tz in timeZones:
                newType, info = etype(
                    issueTime, hazRec['endTime'], tz, 'endTime')
    ...
    def getTimingType(self, hazRec, issueTime):
        ...
        locStart, locEnd = self.getLocalHeadlinesTiming(
            None, None, hazRec['phen'], tr, hazRec['geoId'], issueTime)
...
######################  FROM GFE Header.py
    ...
    def getExpireTime(self, issueTime, purgeHours, hazards, roundMinutes=15,
        fixedExpire=0):
        '''
        Given the issuance time, purgeHours, and the hazards,
        ...
        @param purgeHours -- set time past issuance time. 
               ...
               is used.
               In PV2, the user will be able to adjust this from the
               Hazard Information dialog.
               if -1, then hazard end time is to be used
        @param hazards in the segment
        @param roundMinutes
        ...
        if purgeHours > 0:
            ...
            # Pick the earliest end time of the hazards in the segment
            for hazard in hazards:
                expireTime = None
                if expireTime is None or hazard.get("endTime") < expireTime:
                    expireTime = hazard.get("endTime")
