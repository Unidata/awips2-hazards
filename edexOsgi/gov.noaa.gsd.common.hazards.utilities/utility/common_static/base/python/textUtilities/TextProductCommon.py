...
class TextProductCommon(object):
    ...
    def hazardTimePhrases(self, vtecRecord, creationTime, prefixSpace=True):
        '''
    ...
    def getHeadlinesAndSections(self, vtecRecords, metaDataList, productID, creationTime):
        ...
        while len(hList) > 0:
            ...
            if (vtecRecord['act'] != 'EXP' and creationTime >= vtecRecord['endTime']) or \
            (vtecRecord['act'] == 'EXP' and creationTime > 30*60 + vtecRecord['endTime']):
                hList.remove(vtecRecord)
    ...
    def getTimingPhrase(self, vtecRecord, issueTime, stype=None, etype=None):
        ...
        if type(stype) is types.MethodType:
            for tz in timeZones:
                newType, info = stype(
                    issueTime, vtecRecord['startTime'], tz, 'startTime')
        ...
        if type(etype) is types.MethodType:
            for tz in timeZones:
                newType, info = etype(
                    issueTime, vtecRecord['endTime'], tz, 'endTime')
    ...
    def getTimingType(self, vtecRecord, issueTime):
        ...
        tr = self.makeTimeRange(vtecRecord['startTime'], vtecRecord['endTime'])
        locStart, locEnd = self.getLocalHeadlinesTiming(
            None, None, vtecRecord['phen'], tr, vtecRecord['id'], issueTime)
                
        #time from issuanceTime
        deltaTstart = vtecRecord['startTime'] - issueTime  #seconds past now
        deltaTend = vtecRecord['endTime'] - issueTime  #seconds past now
