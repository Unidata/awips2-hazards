...
'''
 ...
 Note that this module is a duplicate of PythonOverriderCore.py from Awips2_baseline. That
 module is being removed in 17.3.1, so it needed moved into HazardServices. The module name was
 changed to avoid python import issues and having to tweak the include paths.
 
 History:
 Date         Ticket#    Engineer    Description
 ------------ ---------- ----------- --------------------------
 Mar 06, 2017 28300      Robert.Blum Initial Creation (copied from Awips2_baseline)
...
def _buildLocalizationLevelsList(availableLevels, desiredLevel):
    if desiredLevel == None:
        ...
        if desiredLevel not in availableLevels:
            raise LookupError('An invalid level has been specified:' + str(desiredLevel) + '!')
...
def _mergeClasses(source, target, className):
    ...
    for attr in dir(targetClass):
        # skip attributes already in the sourceClass
        if not attr in sourceClass.__dict__:
            if isinstance(getattr(targetClass, attr), types.MethodType):
                ...
                if not (attr.startswith('__') or attr.startswith('_') or \
                    isinstance(attr, types.BuiltinFunctionType)):
                    mergeDirective = 'source.' + className +"." + attr + ' = target.' + className + "." + attr
                    exec(mergeDirective)
