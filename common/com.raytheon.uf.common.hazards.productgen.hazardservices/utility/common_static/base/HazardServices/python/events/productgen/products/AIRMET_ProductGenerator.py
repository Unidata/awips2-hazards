...
###################################################
    ...
    def execute(self, eventSet, dialogInputMap):          
        ...
        parts = ['originatingOffice', 'currentTime', 'zone', 'advisoryType',
                 ...
                 'currentBoundingStatement'] #'outlookTime'
...
######################START ACTUAL PRODUCT GENERATION METHODS ###################################
    ...
    def updateNumberDict(self, updateDict):
        ...
        updateNumberDict = {'issuancePackage': self._latestStartTime,
        ...
        if os.path.isfile('/scratch/airmetNumber.txt'):
            with open('/scratch/airmetNumber.txt') as openFile:
                updateNumberDict = json.load(openFile)  
            oldIssuancePackage = updateNumberDict['issuancePackage']
            #if not same issuance time set to new updateNumberDict
            if oldIssuancePackage != updateNumberDict['issuancePackage']:
                pass
            else:
                ...
                for key, value in updateDict.iteritems():
                    for key1, value1 in value.iteritems():
                        for key2, value2 in value1.iteritems():
                            if value2 == True:
                                updateNumberDict['updateNumber'][key1][key2] += 1

            #if issuing update the dictionary with all of the new entries
            if self._issueFlag == "True":
                with open('/scratch/airmetNumber.txt', 'w') as outFile:
                    json.dump(updateNumberDict, outFile)
