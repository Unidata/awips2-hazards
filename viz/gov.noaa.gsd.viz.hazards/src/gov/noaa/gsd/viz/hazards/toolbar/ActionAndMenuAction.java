...
/**
 * Abstract class from which to derive an action that provides both a main
 * button and a drop-down menu (the latter actually provided by a subclass of
 * type {@link ActionAndMenuHelperAction}, with said type specified by the
 * generic parameter <code>H</code>). The main button shows the image for, and
 * when pressed activates, an action known as the principal. The generic
 * parameter <code>A</code> is the subclass's type, while the generic parameter
 ...
 * @author golden
 */
public abstract class ActionAndMenuAction<A extends ActionAndMenuAction<A, H, P>, H extends ActionAndMenuHelperAction<A, H, P>, P extends BasicAction>
        extends BasicAction {
     ...
     *            Action that is currently the one being wrapped by this action.
     * @param helper
     *            Action that is to serve as a helper for this action; the
     *            helper will provide the drop-down.
     */
    public ActionAndMenuAction(P principal, H helper) {
        super(principal.getText(), principal.getStyle(),
                principal.getImageDescriptor(), principal.getToolTipText());
        this.helperAction = helper;
        helper.setActionAndMenuAction(this);
        setEnabled(principal.isEnabled());
    ...
    /**
     * Get the helper for this action.
     * 
     * @return Helper for this action.
     */
    public H getHelper() {
        return helperAction;
    }
    ...
    public void setEnabled(boolean enable) {
        ...
        helperAction.setEnabled(enable);
    }
