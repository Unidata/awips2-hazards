...
/**
 * Base class for widget delegates, used to provide thread-safe access to
 * {@link Widget SWT widgets} and UI components that are composed of SWT
 * widgets. The generic parameter <code>I</code> provides the type of widget
 ...
 * {@link PrincipalRunnableTask} instances that encapsulate whatever task must
 * be run on the principal, and then pass them to
 * {@link #runOrScheduleTask(PrincipalRunnableTask)}. This will ensure that such
 * tasks are run on the main UI thread, and only when the principal has become
 * available.
 * </p>
 * <p>
 * Implementations of subclass methods that return values should create
 * {@link PrincipalCallableTask} instances that encapsulate whatever task must
 * be run on the principal and whatever value must be returned, and then pass
 * them to {@link #callTask(PrincipalCallableTask)}. In contrast to <code>
 ...
 * @version 1.0
 */
public class WidgetDelegate<I, W extends IWidget<I>> implements IWidget<I> {
     ...
     */
    private final IWidgetDelegateHelper<I, W> helper;
     ...
     *            Handler invocation scheduler.
     */
    public WidgetDelegate(IWidgetDelegateHelper<I, W> helper,
    ...
    @Override
    public void setEnabled(final I identifier, final boolean enable) {
        runOrScheduleTask(new PrincipalRunnableTask<I, W>() {

            @Override
            public void run() {
                getPrincipal().setEnabled(identifier, enable);
            }
        });
    }
    ...
    protected final void runOrScheduleTask(
            final PrincipalRunnableTask<I, W> task) {
     ...
     */
    protected final void runOrScheduleTask(
            final PrincipalRunnableTask<I, W> task, final boolean persist) {
     ...
     */
    protected final <R> R callTask(final PrincipalCallableTask<I, W, R> task) {
