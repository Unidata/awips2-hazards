...
/**
 * Enhanced version of the {@link ActionContributionItem}, with changes made to
 * work with {@link IPulldownArrowButtonOnly} instances. (Search for that class to find
 * the places in the code that have been changed to accommodate said class.)
 * 
 * <pre>
 *
 * SOFTWARE HISTORY
 ...
 * ------------ ---------- ------------- --------------------------
 * Jan 5, 2018    33428    Chris.Golden  Initial creation.
 * </pre>
 *
 * @author Chris.Golden
 */
@SuppressWarnings("restriction")
public class EnhancedActionContributionItem extends ContributionItem {

    /**
     * Width of a down-arrow-only @{link ToolItem}. This is calculated and set
     * if it is found to be <code>0</code> when it is needed.
     */
    private static int downArrowOnlyToolItemWidth;
     ...
     *            the action
     */
    public EnhancedActionContributionItem(IAction action) {
        super(action.getId());
        this.action = action;
    }
    ...
    public boolean equals(Object o) {
        if (!(o instanceof EnhancedActionContributionItem)) {
            return false;
        }
        return action.equals(((EnhancedActionContributionItem) o).action);
    }
     ...
     * has been set, the button is created and primed to the value of the
     * checked property.
     */
    @Override
    ...
    public void fill(ToolBar parent, int index) {
        if (widget == null && parent != null) {
            ...
            /*
             * If the action should only show a pull-down arrow button, not any
             * other button to the left of the arrow button, shrink its width
             * down to be the appropriate value to only show the arrow button.
             */
            if (action instanceof IPulldownArrowButtonOnly) {

                /*
                 * If the down-arrow-only tool item width has not yet been
                 * determined, figure it out by creating a temporary toolbar and
                 * first populating it with a drop-down tool item, then
                 * measuring its width, and then populating the toolbar with a
                 * push-button tool item, and subtracting the latter's width
                 * from the former's.
                 */
                if (downArrowOnlyToolItemWidth == 0) {
                    ToolBar testToolbar = new ToolBar(parent.getShell(),
                            SWT.FLAT);
                    Image image = new Image(Display.getCurrent(), 20, 20);
                    ToolItem item = new ToolItem(testToolbar, SWT.DROP_DOWN);
                    item.setImage(image);
                    downArrowOnlyToolItemWidth = testToolbar
                            .computeSize(SWT.DEFAULT, SWT.DEFAULT).x;
                    item.dispose();
                    item = new ToolItem(testToolbar, SWT.PUSH);
                    item.setImage(image);
                    downArrowOnlyToolItemWidth -= testToolbar
                            .computeSize(SWT.DEFAULT, SWT.DEFAULT).x + 1;
                    item.dispose();
                    testToolbar.dispose();
                }

                /*
                 * Set the width of the tool item to only show the down-arrow
                 * button. This code was copied from ToolItem.resizeHandle()
                 * (which called into Control.gtk_widget_size_request(), and
                 * that code is copied into here as well, since that method is
                 * not visible).
                 */
                OS.gtk_widget_set_size_request(ti.handle,
                        downArrowOnlyToolItemWidth, ti.getBounds().height);
                GtkRequisition requisition = new GtkRequisition();
                if (OS.GTK3) {
                    OS.gtk_widget_get_preferred_size(ti.handle, requisition,
                            null);
                } else {
                    OS.gtk_widget_size_request(ti.handle, requisition);
                }
                GtkAllocation allocation = new GtkAllocation();
                OS.gtk_widget_get_allocation(ti.handle, allocation);
                allocation.width = downArrowOnlyToolItemWidth;
                allocation.height = ti.getBounds().height;
                OS.gtk_widget_size_allocate(ti.handle, allocation);
            }
    ...
    private void handleWidgetSelection(Event e, boolean selection) {
        ...
        if (item != null) {
            ...
            if (action.isEnabled()) {
                ...
                if (callback != null) {
                    callback.notEnabled(action, new NotEnabledException(
                            action.getText() + " is not enabled.")); //$NON-NLS-1$
                }
            }
        }
    }
    ...
    private boolean updateImages(boolean forceImage) {
        ...
        if (widget instanceof ToolItem) {
            if (USE_COLOR_ICONS) {
                ...
                if (action instanceof IPulldownArrowButtonOnly == false) {
                    ((ToolItem) widget)
                            .setDisabledImage(disabledImage == null ? null
                                    : localManager.createImageWithDefault(
                                            disabledImage));
                ...
                if (!event.widget.isDisposed()) {
                    ...
                    if (!realItem.isDisposed()) {
                        ...
                        if (event.type == SWT.Selection
                                && ((style & (SWT.TOGGLE | SWT.CHECK
                                        | SWT.RADIO)) != 0)
                                && realItem instanceof MenuItem) {
                            ((MenuItem) realItem).setSelection(
