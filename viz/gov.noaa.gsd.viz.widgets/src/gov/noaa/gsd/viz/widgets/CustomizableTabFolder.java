...
/**
 * Description: Customizable tab folder, to be used with a
 * {@link CustomizableTabItem}.
 * <p>
 * Note that this class is a copy of {@link CTabFolder} changed to work with a
 * <code>CustomizableTabItem</code> instead of a {@link CTabItem}. The
 * motivation for creating these barely-different versions of the
 * <code>CTabXXXX</code> classes is the addition of per-tab foreground (text)
 * color configuration to <code>CTabItem</code>. If SWT was designed with
 * subclassing in mind, this would properly have been approached by subclassing
 * {@link CTabFolderRenderer} and customizing it as necessary, but this was
 * impossible due to the tight coupling between the various
 * <code>CTabXXXX</code> classes, including use of one another's package-private
 * member variables, together with the fact that most of said member variables
 * are not accessible to subclasses.
 * </p>
 * 
 * <pre>
 * 
 ...
 * @version 1.0
 */
public class CustomizableTabFolder extends Composite {
    ...
    /* item management */
    CustomizableTabFolderRenderer renderer;

    CustomizableTabItem items[] = new CustomizableTabItem[0];
    ...
    ICustomizableTabFolderListener[] folderListeners = new ICustomizableTabFolderListener[0];
    ...
    // when disposing CustomizableTabFolder, don't try to layout the items or
     ...
     * @see #getStyle()
     */
    public CustomizableTabFolder(Composite parent, int style) {
        super(parent, checkStyle(parent, style));
        init(style);
    }

    void init(int style) {
        super.setLayout(new CustomizableTabFolderLayout());
        ...
        renderer = new CustomizableTabFolderRenderer(this);
    ...
    static int checkStyle(Composite parent, int style) {
         ...
         * the CustomizableTabFolder not drawing correctly. To alleviate some of
         * the appearance problems, allow the OS to draw the background. This
         * does not draw correctly but the result is less obviously wrong.
         */
     ...
     *                </ul>
     * 
     * @see ICustomizableTabFolderListener
     * @see #removeCustomizableTabFolderListener(ICustomizableTabFolderListener)
     * 
     * @since 3.0
     */
    public void addCustomizableTabFolderListener(
            ICustomizableTabFolderListener listener) {
        checkWidget();
        if (listener == null) {
            SWT.error(SWT.ERROR_NULL_ARGUMENT);
        }
        // add to array
        ICustomizableTabFolderListener[] newListeners = new ICustomizableTabFolderListener[folderListeners.length + 1];
    ...
    public Rectangle computeTrim(int x, int y, int width, int height) {
        checkWidget();
        return renderer.computeTrim(CustomizableTabFolderRenderer.PART_BODY,
                SWT.NONE, x, y, width, height);
    }

    void createItem(CustomizableTabItem item, int index) {
        if (0 > index || index > getItemCount()) {
            SWT.error(SWT.ERROR_INVALID_RANGE);
        }
        item.parent = this;
        CustomizableTabItem[] newItems = new CustomizableTabItem[items.length + 1];
    ...
    void destroyItem(CustomizableTabItem item) {
        if (inDispose) {
            return;
        }
        ...
        if (items.length == 1) {
            items = new CustomizableTabItem[0];
            priority = new int[0];
            firstIndex = -1;
            selectedIndex = -1;
    ...
    public Rectangle getClientArea() {
        checkWidget();
        Rectangle trim = renderer.computeTrim(
                CustomizableTabFolderRenderer.PART_BODY, SWT.NONE, 0, 0, 0, 0);
        if (minimized) {
     ...
     *                </ul>
     */
    public CustomizableTabItem getItem(int index) {
        // checkWidget();
     ...
     * 
     * @param pt
     *            the point in coordinates relative to the CustomizableTabFolder
     ...
     *                </ul>
     */
    public CustomizableTabItem getItem(Point pt) {
        // checkWidget();
        if (items.length == 0) {
            return null;
        }
        Point size = getSize();
        Rectangle trim = renderer
                .computeTrim(CustomizableTabFolderRenderer.PART_BORDER,
                        SWT.NONE, 0, 0, 0, 0);
        ...
        for (int i = 0; i < priority.length; i++) {
            CustomizableTabItem item = items[priority[i]];
     ...
     *                </ul>
     */
    public CustomizableTabItem[] getItems() {
        // checkWidget();
        CustomizableTabItem[] tabItems = new CustomizableTabItem[items.length];
     ...
     * tabs are visible.
     * <p>
     * For example, consider a CustomizableTabFolder that contains "Tab 1",
     * "Tab 2", "Tab 3" and "Tab 4" (in order by index). The user selects
     * "Tab 1" and then "Tab 3". If the CustomizableTabFolder is now compressed
     * so that only two tabs are visible, by default, "Tab 2" and "Tab 3" will
     * be shown ("Tab 3" since it is currently selected and "Tab 2" because it
     * is the previous item in index order). If MRU visibility is enabled, the
     * two visible tabs will be "Tab 1" and "Tab 3" (in that order from left to
     * right).
     * </p>
     * 
     ...
     *                </ul>
     * 
     * @see #setRenderer(CustomizableTabFolderRenderer)
     * @see CustomizableTabFolderRenderer
     * 
     * @since 3.6
     */
    public CustomizableTabFolderRenderer getRenderer() {
        checkWidget();
        return renderer;
    }
    ...
    int getRightItemEdge(GC gc) {
        Rectangle trim = renderer
                .computeTrim(CustomizableTabFolderRenderer.PART_BORDER,
                        SWT.NONE, 0, 0, 0, 0);
        int x = getSize().x - (trim.width + trim.x) - 3; // TODO: add setter for
                                                         // spacing?
        if (showMin) {
            x -= renderer.computeSize(
     ...
     *                </ul>
     */
    public CustomizableTabItem getSelection() {
        // checkWidget();
        if (selectedIndex == -1) {
            return null;
        }
    ...
    /**
     * Returns <code>true</code> if the CustomizableTabFolder is rendered with a
     * simple, traditional shape.
     * 
     * @return <code>true</code> if the CustomizableTabFolder is rendered with a
     *         simple shape
     * 
     * @since 3.0
     */
    ...
    /**
     * Returns <code>true</code> if the CustomizableTabFolder only displays the
     * selected tab and <code>false</code> if the CustomizableTabFolder displays
     * multiple tabs.
     * 
     * @return <code>true</code> if the CustomizableTabFolder only displays the
     *         selected tab and <code>false</code> if the CustomizableTabFolder
     ...
     *                </ul>
     */
    public int indexOf(CustomizableTabItem item) {
        checkWidget();
        if (item == null) {
            SWT.error(SWT.ERROR_NULL_ARGUMENT);
        }
                ...
                if (childID >= 0 && childID < items.length) {
                    ...
                    if (text != null) {
                        ...
                        if (mnemonic != '\0') {
                            shortcut = SWT
                                    .getMessage(
                                            "SWT_Page_Mnemonic", new Object[] { new Character(mnemonic) }); //$NON-NLS-1$
                        }
                    }
                }
                if (childID == ACC.CHILDID_SELF) {
        ...
        case SWT.ARROW_RIGHT:
            ...
            if (!mru) {
                ...
                if (current + offset >= 0 && current + offset < idx) {
                    ...
                    if (showChevron) {
                        CustomizableTabFolderEvent e = new CustomizableTabFolderEvent(
                                this);
                        e.widget = this;
                        e.time = event.time;
                        e.x = chevronRect.x;
    ...
    void onMouse(Event event) {
        ...
        case SWT.MouseExit: {
            ...
            for (int i = 0; i < items.length; i++) {
                CustomizableTabItem item = items[i];
                if (i != selectedIndex
        ...
        case SWT.MouseMove: {
            ...
            for (int i = 0; i < items.length; i++) {
                ...
                if (i != selectedIndex
                        && item.closeImageState != SWT.BACKGROUND && !close) {
                    item.closeImageState = SWT.BACKGROUND;
                    redraw(item.closeRect.x, item.closeRect.y,
                            item.closeRect.width, item.closeRect.height, false);
                }
            ...
            if (item != null) {
                if (item.closeRect.contains(x, y)) {
                    ...
                    for (int j = 0; j < folderListeners.length; j++) {
                        ICustomizableTabFolderListener listener = folderListeners[j];
                        listener.close(e);
                    }
                    if (e.doit) {
                        item.dispose();
                    ...
                    if (!isDisposed() && item.isDisposed()) {
                        ...
                        CustomizableTabItem nextItem = getItem(pt);
                        if (nextItem != null) {
    ...
    void onPaint(Event event) {
        ...
        renderer.draw(CustomizableTabFolderRenderer.PART_BODY, SWT.BACKGROUND
                | SWT.FOREGROUND, bodyRect, gc);

        gc.setFont(gcFont);
        gc.setForeground(gcForeground);
        gc.setBackground(gcBackground);

        renderer.draw(CustomizableTabFolderRenderer.PART_HEADER, SWT.BACKGROUND
                | SWT.FOREGROUND, bodyRect, gc);
        ...
        if (!single) {
            for (int i = 0; i < items.length; i++) {
                ...
                if (i != selectedIndex
                        && event.getBounds().intersects(itemBounds)) {
                    renderer.draw(i, SWT.BACKGROUND | SWT.FOREGROUND
                            | items[i].state, itemBounds, gc);
                }
            }
        }
        ...
        if (selectedIndex != -1) {
            renderer.draw(selectedIndex, items[selectedIndex].state
        ...
        renderer.draw(CustomizableTabFolderRenderer.PART_MAX_BUTTON,
                maxImageState, maxRect, gc);
        renderer.draw(CustomizableTabFolderRenderer.PART_MIN_BUTTON,
                minImageState, minRect, gc);
        renderer.draw(CustomizableTabFolderRenderer.PART_CHEVRON_BUTTON,
                chevronImageState, chevronRect, gc);
    ...
    void onResize() {
        ...
        if (oldSize == null) {
            ...
            if (onBottom && size.y != oldSize.y) {
                ...
                Rectangle trim = renderer.computeTrim(
                        CustomizableTabFolderRenderer.PART_BODY, SWT.NONE, 0,
                        0, 0, 0);
                if (size.x != oldSize.x) {
     ...
     *                </ul>
     * 
     * @see #addCustomizableTabFolderListener(ICustomizableTabFolderListener)
     * 
     * @since 3.0
     */
    public void removeCustomizableTabFolderListener(
            ICustomizableTabFolderListener listener) {
        checkWidget();
        if (listener == null) {
            SWT.error(SWT.ERROR_NULL_ARGUMENT);
        ...
        if (folderListeners.length == 1) {
            folderListeners = new ICustomizableTabFolderListener[0];
            return;
        }
        ICustomizableTabFolderListener[] newTabListeners = new ICustomizableTabFolderListener[folderListeners.length - 1];
     ...
     * @param colors
     *            an array of Color that specifies the colors to appear in the
     *            gradient in order of appearance from top to bottom or left to
     *            right. The value <code>null</code> clears the background
     *            gradient. The value <code>null</code> can be used inside the
    ...
    void setButtonBounds(GC gc) {
        ...
        if (showMax) {
            Point maxSize = renderer.computeSize(
                    CustomizableTabFolderRenderer.PART_MAX_BUTTON, SWT.NONE,
                    gc, SWT.DEFAULT, SWT.DEFAULT);
            maxRect.x = size.x - borderRight - maxSize.x - 3;
            if (borderRight > 0) {
                maxRect.x += 1;
            }
            maxRect.y = onBottom ? size.y - borderBottom - tabHeight
                    + (tabHeight - maxSize.y) / 2 : borderTop
                    + (tabHeight - maxSize.y) / 2;
        ...
        if (showMin) {
            Point minSize = renderer.computeSize(
                    CustomizableTabFolderRenderer.PART_MIN_BUTTON, SWT.NONE,
                    gc, SWT.DEFAULT, SWT.DEFAULT);
            ...
            minRect.y = onBottom ? size.y - borderBottom - tabHeight
                    + (tabHeight - minSize.y) / 2 : borderTop
                    + (tabHeight - minSize.y) / 2;
            ...
            case SWT.FILL: {
                ...
                if (single) {
                    if (items.length == 0 || selectedIndex == -1) {
                        ...
                        CustomizableTabItem item = items[selectedIndex];
                        int chevronWidth = renderer
                                .computeSize(
                                        CustomizableTabFolderRenderer.PART_CHEVRON_BUTTON,
                                        SWT.NONE, gc, SWT.DEFAULT, SWT.DEFAULT).x;
                        if (item.x + item.width + 7 + chevronWidth >= rightEdge) {
                            break;
                        }
                    ...
                    if (items.length == 0) {
                        ...
                        CustomizableTabItem lastItem = items[lastIndex];
        ...
        Point chevronSize = renderer.computeSize(
                CustomizableTabFolderRenderer.PART_CHEVRON_BUTTON, SWT.NONE,
                gc, SWT.DEFAULT, SWT.DEFAULT);
        if (single) {
            ...
            if (showChevron) {
                ...
                if (!simple && lastIndex == selectedIndex) {
                    w -= renderer.curveIndent; // TODO: fix chevron position
                }
     ...
     *                </ul>
     */
    public void setInsertMark(CustomizableTabItem item, boolean after) {
        checkWidget();
    }
    ...
    boolean setItemLocation(GC gc) {
        ...
        int y = onBottom ? Math.max(borderBottom, size.y - borderBottom
                - tabHeight) : borderTop;
        Point closeButtonSize = renderer.computeSize(
                CustomizableTabFolderRenderer.PART_CLOSE_BUTTON, 0, gc,
        ...
        if (single) {
                ...
                if (i == selectedIndex) {
                    ...
                    if (showClose || item.showClose) {
                        ...
                        item.closeRect.y = onBottom ? size.y - borderBottom
                                - tabHeight + (tabHeight - closeButtonSize.y)
                                / 2 : borderTop
            ...
            int x = -renderer.computeTrim(
                    CustomizableTabFolderRenderer.PART_HEADER, SWT.NONE, 0, 0,
                    0, 0).x;
            int defaultX = getDisplay().getBounds().width + 10; // off screen
            ...
            for (int i = 0; i < items.length; i++) {
                ...
                if (!item.showing) {
                    ...
                    Rectangle edgeTrim = renderer.computeTrim(i, state, 0, 0,
                            0, 0);
                    item.closeRect.x = item.x + item.width
                            - (edgeTrim.width + edgeTrim.x) - closeButtonSize.x;
                    item.closeRect.y = onBottom ? size.y - borderBottom
                            - tabHeight + (tabHeight - closeButtonSize.y) / 2
                            : borderTop + (tabHeight - closeButtonSize.y) / 2;
                    x = x + item.width;
                    if (!simple && i == selectedIndex) {
                        x -= renderer.curveIndent; // TODO: fix next item
                                                   // position
                    }
                }
            }
    ...
    boolean setItemSize(GC gc) {
        ...
        if (single) {
            showChevron = true;
            if (selectedIndex != -1) {
                CustomizableTabItem tab = items[selectedIndex];
                int width = renderer.computeSize(selectedIndex, SWT.SELECTED,
                        gc, SWT.DEFAULT, SWT.DEFAULT).x;
                ...
                if (tab.height != tabHeight || tab.width != width) {
                    ...
                    if (showClose || tab.showClose) {
                        Point closeSize = renderer
                                .computeSize(
                                        CustomizableTabFolderRenderer.PART_CLOSE_BUTTON,
                                        SWT.SELECTED, gc, SWT.DEFAULT,
                                        SWT.DEFAULT);
        ...
        if (showMin) {
            tabAreaWidth -= renderer.computeSize(
                    CustomizableTabFolderRenderer.PART_MIN_BUTTON, SWT.NONE,
                    gc, SWT.DEFAULT, SWT.DEFAULT).x;
        }
        if (showMax) {
        ...
        for (int i = 0; i < priority.length; i++) {
            ...
            int state = CustomizableTabFolderRenderer.MINIMUM_SIZE;
            if (index == selectedIndex) {
                state |= SWT.SELECTED;
            }
            minWidths[index] = renderer.computeSize(index, state, gc,
        ...
        if (minWidth > tabAreaWidth) {
            ...
            for (int i = 0; i < items.length; i++) {
                ...
                maxWidths[i] = renderer.computeSize(i, state, gc, SWT.DEFAULT,
                        SWT.DEFAULT).x;
        ...
        for (int i = 0; i < items.length; i++) {
            CustomizableTabItem tab = items[i];
            int width = widths[i];
            if (tab.height != tabHeight || tab.width != width) {
                changed = true;
                tab.shortenedText = null;
                ...
                if (showClose || tab.showClose) {
                    if (i == selectedIndex || showUnselectedClose) {
                        Point closeSize = renderer
                                .computeSize(
                                        CustomizableTabFolderRenderer.PART_CLOSE_BUTTON,
                                        SWT.NONE, gc, SWT.DEFAULT, SWT.DEFAULT);
                        tab.closeRect.width = closeSize.x;
                        tab.closeRect.height = closeSize.y;
                    }
                }
     ...
     * @since 3.6
     */
    public void setRenderer(CustomizableTabFolderRenderer renderer) {
        checkWidget();
     ...
     *                </ul>
     */
    public void setSelection(CustomizableTabItem item) {
        checkWidget();
        if (item == null) {
            SWT.error(SWT.ERROR_NULL_ARGUMENT);
        }
    ...
    public void setSelection(int index) {
        ...
        CustomizableTabItem selection = items[index];
        if (selectedIndex == index) {
            showItem(selection);
            return;
        }
     ...
     * 
     * <pre>
     * cfolder.setSelectionBackground(
     *         new Color[] { display.getSystemColor(SWT.COLOR_DARK_BLUE),
     *                 display.getSystemColor(SWT.COLOR_BLUE),
     *                 display.getSystemColor(SWT.COLOR_WHITE),
     *                 display.getSystemColor(SWT.COLOR_WHITE) }, new int[] { 25, 50,
     ...
     * @param colors
     *            an array of Color that specifies the colors to appear in the
     *            gradient in order of appearance from top to bottom. The value
     *            <code>null</code> clears the background gradient. The value
     *            <code>null</code> can be used inside the array of Color to
     *            specify the background color. For vertical gradients, the
     *            colors array can optionally have an extra entry at the end to
     *            specify a highlight top color.
     * @param percents
     *            an array of increasing integers between 0 and 100 specifying
     *            the percent of the width of the widget at which the color
     *            should change. The size of the percents array must be one less
     *            than the size of the colors array, unless there is a highlight
     *            top color, in which case it must be exactly two less than the
    ...
    /**
     * Sets the shape that the CustomizableTabFolder will use to render itself.
     * 
     * @param simple
     *            <code>true</code> if the CustomizableTabFolder should render
     *            itself in a simple, traditional style
     * 
     * @exception SWTException
     *                <ul>
     *                <li>ERROR_WIDGET_DISPOSED - if the receiver has been
    ...
    /**
     * Sets the number of tabs that the CustomizableTabFolder should display
     * 
     * @param single
    ...
    public void setSingle(boolean single) {
        ...
        if (this.single != single) {
            ...
            if (!single) {
                for (int i = 0; i < items.length; i++) {
                    if (i != selectedIndex
                            && items[i].closeImageState == SWT.NONE) {
     ...
     *                of this CustomizableTabFolder</li>
     *                </ul>
     * 
     * @since 2.1
     */
     ...
     *                </ul>
     * 
     * @see CustomizableTabFolder#showSelection()
     * 
     * @since 2.0
     */
    public void showItem(CustomizableTabItem item) {
        checkWidget();
        if (item == null) {
            SWT.error(SWT.ERROR_NULL_ARGUMENT);
        }
    ...
    void showList(Rectangle rect) {
        ...
        if (showMenu == null || showMenu.isDisposed()) {
            showMenu = new Menu(getShell(), getStyle()
        ...
        final String id = "CustomizableTabFolder_showList_Index"; //$NON-NLS-1$
        for (int i = 0; i < items.length; i++) {
            CustomizableTabItem tab = items[i];
            if (tab.showing) {
                continue;
            ...
            item.addSelectionListener(new SelectionAdapter() {
                ...
                public void widgetSelected(SelectionEvent e) {
                    ...
                    int index = indexOf((CustomizableTabItem) menuItem
                            .getData(id));
                    CustomizableTabFolder.this.setSelection(index, true);
                }
            });
        }
        int x = rect.x;
     ...
     *                </ul>
     * 
     * @see CustomizableTabFolder#showItem(CustomizableTabItem)
     * 
     * @since 2.0
     */
    ...
    boolean updateItems(int showIndex) {
        ...
        if (!single && !mru && showIndex != -1) {
            ...
            if (priority[0] < showIndex) {
                Rectangle trim = renderer.computeTrim(
                        CustomizableTabFolderRenderer.PART_BORDER, SWT.NONE, 0,
                        0, 0, 0);
                int borderLeft = -trim.x;
    ...
    boolean updateTabHeight(boolean force) {
        ...
        GC gc = new GC(this);
        tabHeight = renderer.computeSize(
                CustomizableTabFolderRenderer.PART_HEADER, SWT.NONE, gc,
    ...
    String _getToolTip(int x, int y) {
        ...
        CustomizableTabItem item = getItem(new Point(x, y));
        if (item == null) {
            return null;
        }
        if (!item.showing) {
