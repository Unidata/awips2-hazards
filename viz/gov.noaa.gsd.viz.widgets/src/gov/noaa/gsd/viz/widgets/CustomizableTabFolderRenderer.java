...
/**
 * Description: Customizable tab folder renderer, to be used with a
 * {@link CustomizableTabFolder}.
 * <p>
 * Note that this class is a copy of {@link CTabFolderRenderer}, with the
 * addition of the ability to render the tab text using individual tab items'
 * foreground colors, and of course changed to work with a
 * <code>CustomizableTabFolder</code> instead of a {@link CTabFolder}. Changes
 * are marked with a "CHANGED" comment.
 * </p>
 * 
 * <pre>
 * 
 ...
 * @version 1.0
 */
public class CustomizableTabFolderRenderer {

    protected CustomizableTabFolder parent;
    ...
    /*
     * Border color that was used in computing the cached anti-alias Colors. We
     * have to recompute the colors if the border color changes
     */
    ...
    static final int[] TOP_LEFT_CORNER_HILITE = new int[] { 5, 2, 4, 2, 3, 3,
            2, 4, 2, 5, 1, 6 };
    ...
    static final int[] TOP_LEFT_CORNER_BORDERLESS = new int[] { 0, 6, 1, 5, 1,
            4, 4, 1, 5, 1, 6, 0 };

    static final int[] TOP_RIGHT_CORNER_BORDERLESS = new int[] { -7, 0, -6, 1,
            -5, 1, -2, 4, -2, 5, -1, 6 };

    static final int[] BOTTOM_LEFT_CORNER_BORDERLESS = new int[] { 0, -6, 1,
            -6, 1, -5, 2, -4, 4, -2, 5, -1, 6, -1, 6, 0 };

    static final int[] BOTTOM_RIGHT_CORNER_BORDERLESS = new int[] { -7, 0, -7,
            -1, -6, -1, -5, -2, -3, -4, -2, -5, -2, -6, -1, -6 };

    static final int[] SIMPLE_TOP_LEFT_CORNER_BORDERLESS = new int[] { 0, 2, 1,
            1, 2, 0 };

    static final int[] SIMPLE_TOP_RIGHT_CORNER_BORDERLESS = new int[] { -3, 0,
            -2, 1, -1, 2 };

    static final int[] SIMPLE_BOTTOM_LEFT_CORNER_BORDERLESS = new int[] { 0,
            -3, 1, -2, 2, -1, 3, 0 };

    static final int[] SIMPLE_BOTTOM_RIGHT_CORNER_BORDERLESS = new int[] { -4,
            0, -3, -1, -2, -2, -1, -3 };
    ...
    public static final int MINIMUM_SIZE = 1 << 24; // TODO: Should this be a
                                                    // state?
     ...
     * @see Widget#getStyle
     */
    public CustomizableTabFolderRenderer(CustomizableTabFolder parent) {
        if (parent == null) {
            return;
        }
    ...
    protected Point computeSize(int part, int state, GC gc, int wHint, int hHint) {
        ...
        case PART_HEADER:
            if (parent.fixedTabHeight != SWT.DEFAULT) {
                height = parent.fixedTabHeight == 0 ? 0
                        : parent.fixedTabHeight + 1; // +1 for line drawn across
                                                     // top of tab
            } else {
                CustomizableTabItem[] items = parent.items;
                if (items.length == 0) {
        ...
        default:
            if (0 <= part && part < parent.getItemCount()) {
                updateCurves();
                CustomizableTabItem item = parent.items[part];
                if (item.isDisposed()) {
                ...
                if ((state & MINIMUM_SIZE) != 0) {
                    ...
                    if (text != null && text.length() > minChars) {
                        if (useEllipses()) {
                            int end = minChars < ELLIPSIS.length() + 1 ? minChars
    ...
    protected Rectangle computeTrim(int part, int state, int x, int y,
        ...
        case PART_BODY:
            ...
            width = width + borderLeft + borderRight + 2 * marginWidth + 2
                    * highlight_margin;
            if (parent.minimized) {
                y = parent.onBottom ? y - borderTop : y - highlight_header
                        - tabHeight - borderTop;
                height = borderTop + borderBottom + tabHeight
                        + highlight_header;
            } else {
                y = parent.onBottom ? y - marginHeight - highlight_margin
                        - borderTop : y - marginHeight - highlight_header
                        - tabHeight - borderTop;
                height = height + borderTop + borderBottom + 2 * marginHeight
    ...
    void createSelectionHighlightGradientColors(Color start) {
        ...
        for (int i = 0; i < fadeGradientSize; i++) {
            ...
            selectionHighlightGradientColorsCache[i] = getColor(display,
    ...
    void drawBackground(GC gc, Rectangle bounds, int state) {
        ...
        Color defaultBackground = selected ? parent.selectionBackground
        ...
        if (!parent.minimized) {
            ...
            if (highlight_margin > 0) {
                ...
                if (selectedIndex != -1
                    ...
                    gc.setBackground(selectedIndex == -1 ? parent
                            .getBackground() : parent.selectionBackground);
    ...
    void drawChevron(GC gc, Rectangle chevronRect, int chevronImageState) {
        ...
        case SWT.NONE: {
            Color chevronBorder = parent.single ? parent
                    .getSelectionForeground() : parent.getForeground();
    ...
    void drawHighlight(GC gc, Rectangle bounds, int state, int rightEdge) {
        ...
        gc.drawLine(TOP_LEFT_CORNER_HILITE[0] + x + 1, // rely on fact that
                                                       // first pair is
                                                       // top/right of curve
        ...
        for (int i = 0; i < topCurveHighlightStart.length / 2; i++) {
            ...
            if (lastColorIndex >= gradientsSize) {
                break; // can happen if tabs are unusually short and cut off the
                       // curve
            }
            gc.setForeground(gradients[lastColorIndex]);
            gc.drawPoint(lastX, lastY);
        ...
        for (int i = 0; i < topCurveHighlightEnd.length / 2; i++) {
            int rawX = topCurveHighlightEnd[i * 2]; // d is already encoded in
                                                    // this value
    ...
    void drawSelected(int itemIndex, GC gc, Rectangle bounds, int state) {
        CustomizableTabItem item = parent.items[itemIndex];
        int x = bounds.x;
        int y = bounds.y;
        int height = bounds.height;
        int width = bounds.width;
            ...
            if (parent.single) {
                ...
                if (parent.onBottom) {
                    int[] left = parent.simple ? SIMPLE_BOTTOM_LEFT_CORNER
                            : BOTTOM_LEFT_CORNER;
                    int[] right = parent.simple ? SIMPLE_BOTTOM_RIGHT_CORNER
                            : curve;
                    ...
                    int index = 0;
                    shape[index++] = x; // first point repeated here because
                                        // below we reuse shape to draw outline
                    shape[index++] = y - 1;
                    shape[index++] = x;
                    shape[index++] = y - 1;
                    for (int i = 0; i < left.length / 2; i++) {
                    ...
                    int[] left = parent.simple ? SIMPLE_TOP_LEFT_CORNER
                            : TOP_LEFT_CORNER;
                    int[] right = parent.simple ? SIMPLE_TOP_RIGHT_CORNER
                            : curve;
                    if (borderLeft == 0 && itemIndex == parent.firstIndex) {
                        left = new int[] { x, y };
                    }
                ...
                Color borderColor = parent.getDisplay().getSystemColor(
                        BORDER1_COLOR);
        ...
        if ((state & SWT.FOREGROUND) != 0) {
            ...
            if (textWidth > 0) {
                ...
                if (item.shortenedText == null
                    ...
                    item.shortenedText = shortenText(gc, item.getText(),
                            textWidth);
                    item.shortenedTextWidth = textWidth;
                }
                ...
                /*
                 * CHANGED: Use the item's selection foreground if it has one,
                 * otherwise simply use the parent's selection foreground.
                 */
                gc.setForeground(item.getSelectionForeground() == null ? parent
                        .getSelectionForeground() : item
                ...
                if (parent.isFocusControl()) {
                    ...
                    if (parent.simple || parent.single) {
                        ...
                        gc.setForeground(display.getSystemColor(BUTTON_BORDER));
                        gc.drawLine(xDraw, textY + extent.y + 1, xDraw
                                + extent.x + 1, textY + extent.y + 1);
                    }
                }
            }
            if (parent.showClose || item.showClose) {
    ...
    void drawUnselected(int index, GC gc, Rectangle bounds, int state) {
        CustomizableTabItem item = parent.items[index];
        int x = bounds.x;
        int y = bounds.y;
        int height = bounds.height;
        int width = bounds.width;
        ...
        if ((state & SWT.FOREGROUND) != 0) {
            ...
            if (textWidth > 0) {
                ...
                /*
                 * CHANGED: Use the item's foreground if it has one, otherwise
                 * simply use the parent's foreground.
                 */
                gc.setForeground(item.getForeground() == null ? parent
                        .getForeground() : item.getForeground());
                gc.drawText(item.shortenedText, xDraw, textY, FLAGS);
                gc.setFont(gcFont);
            }
            // draw close
    ...
    void updateCurves() {
        ...
        if (this.getClass()
                .getName()
                .equals("org.eclipse.e4.ui.workbench.renderers.swt.CTabRendering")) {
            return;
        }
        int tabHeight = parent.tabHeight;
