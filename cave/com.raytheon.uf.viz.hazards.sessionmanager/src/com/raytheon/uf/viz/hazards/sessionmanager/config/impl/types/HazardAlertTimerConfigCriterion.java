...
/**
 * Description: Alert configuration for a count-down timer that
 * appears in the Hazard Services console.
 * 
 * <pre>
 * 
 * SOFTWARE HISTORY
...
public class HazardAlertTimerConfigCriterion {
	...
	SPATIAL("Spatial"), CONSOLE("Console");
    ...
    /**
     * Interesting point. fontSize only makes sense for count-down
     * timers in the spatial display. So in the ideal code design
     * world, there would be a config class just for spatial display
     * timers that includes this field. However, then that class has
     * to extend a base count-down timer class. That's problematic
     * when using JAXB because the XML ends up containing references
     * to a class name.
     */
    @XmlElement
    ...
    HazardAlertTimerConfigCriterion(String name, Units units,
	    Set<Location> locations, long millisBeforeExpiration, Color color,
	...
	for (String location : locations) {
	    result.add(Location.fromString(location));
	}
	return result;
    }
